...
PN5180::PN5180(uint8_t MISOpin, uint8_t MOSIpin,uint8_t CLKpin, uint8_t SSpin, uint8_t BUSYpin, uint8_t RSTpin) {
  PN5180_MISO = MISOpin;
  PN5180_MOSI = MOSIpin;
  PN5180_CLK = CLKpin;
  PN5180_NSS = SSpin;
...
void PN5180::begin(SPIClass* _spi) {
...
#if defined(ESP32)
  spiH->begin(PN5180_CLK,PN5180_MISO,PN5180_MOSI,PN5180_NSS);
#else
  spiH->begin();
#endif
  //spiH->setHwCs(true);
...
bool PN5180::writeRegister(uint8_t reg, uint32_t value) {
  ...
  spiH->beginTransaction(PN5180_SPI_SETTINGS);
...
bool PN5180::sendData(uint8_t *data, int len, uint8_t validBits) {
  ...
  if (PN5180_TS_WaitTransmit != transceiveState) {
    ...
    Serial.printf("E TransceiveState = %i\n", transceiveState);
    return false;
  }
...
bool PN5180::setRF_on() {
  ...
  while (0 == (TX_RFON_IRQ_STAT & getIRQStatus()) && millis()-t_start < 100); // wait for RF field to set up
...
bool PN5180::setRF_off() {
  ...
  uint32_t t_start = millis();
  while (0 == (TX_RFOFF_IRQ_STAT & getIRQStatus()) && millis()-t_start < 100); // wait for RF field to shut down
...
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  ...
  uint32_t t_start = millis();
  // 0.
  while (LOW != digitalRead(PN5180_BUSY) && millis()-t_start < 100); // wait until busy is low
  // 1.
  digitalWrite(PN5180_NSS, LOW);// delay(2);
  // 2.
  for (uint8_t i=0; i<sendBufferLen; i++) {
    spiH->transfer(sendBuffer[i]);
  }
  t_start = millis();
  // 3.
  while(HIGH != digitalRead(PN5180_BUSY) && millis()-t_start < 100);  // wait until BUSY is high
  // 4.
  digitalWrite(PN5180_NSS, HIGH); //delay(1);
  t_start = millis();
  // 5.
  while (LOW != digitalRead(PN5180_BUSY) && millis()-t_start < 100); // wait unitl BUSY is low
  ...
  for (uint8_t i=0; i<recvBufferLen; i++) {
    recvBuffer[i] = spiH->transfer(0xff);
  }
  t_start = millis();
  // 3.
  while(HIGH != digitalRead(PN5180_BUSY)  && millis()-t_start < 100);  // wait until BUSY is high
...
void PN5180::reset() {
  ...
  uint32_t t_start = millis();
  while (0 == (IDLE_IRQ_STAT & getIRQStatus())  && millis()-t_start < 100); // wait for system to start up
