...
PN5180ISO15693::PN5180ISO15693(uint8_t MISOpin, uint8_t MOSIpin,uint8_t CLKpin, uint8_t SSpin, uint8_t BUSYpin, uint8_t RSTpin)
...
ISO15693ErrorCode PN5180ISO15693::issueISO15693Command(uint8_t *cmd, uint8_t cmdLen, uint8_t **resultPtr) {
  ...
  while( !(irqR & RX_IRQ_STAT) && millis()-start < 100) {
  delay(1);
  irqR = getIRQStatus();
  }
...
bool PN5180ISO15693::setupRF(uint8_t tx_c, uint8_t rx_c) {
  _tx_c = tx_c;
  _rx_c = rx_c;
  PN5180DEBUG(F("Loading RF-Configuration...\n"));
  if (loadRFConfig(_tx_c, _rx_c)) {  // ISO15693 parameters
    PN5180DEBUG(F("done.\n"));
  }
  else return false;
